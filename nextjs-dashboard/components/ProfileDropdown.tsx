...
export default function ProfileDropdown({ username: propUsername, onLogout: propOnLogout, inline = false }: ProfileDropdownProps) {
  ...
  // ✅ Initialize user auth + username from JWT
  useEffect(() => {
    setIsClient(true);
  ...
  useEffect(() => {
    ...
    const fetchProfilePic = async () => {
      try {
        const res = await fetch(`http://localhost:8000/images/${username}_profile`);
        if (!res.ok) return;
        const data = await res.json();
        const img = `data:${data.contentType};base64,${data.data}`;
        setProfilePic(img);
        localStorage.setItem(cacheKey, img); // cache for later
      } catch (err) {
  ...
  return (
    <div
      className={inline ? "relative" : "fixed top-4 right-4 z-[60]"}
      ref={dropdownRef}
    >
      <button
        ...
        <div className="flex items-center space-x-2">
          ...
          {profilePic ? (
            <img
              src={profilePic}
              alt="profile"
              className="w-6 h-6 md:w-8 md:h-8 rounded-full border object-cover"
            />
          ) : (
      ...
      {isOpen && (
        <div className={`absolute ${inline ? 'right-0' : 'right-0'} mt-2 w-48 md:w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-[60]`}>
          ...
          <button
            ...
            className="w-full text-left px-3 md:px-4 py-2 text-xs md:text-sm text-gray-700 hover:bg-gray-50 flex items-center space-x-2"
          >
          ...
          <button
            ...
            className="w-full text-left px-3 md:px-4 py-2 text-xs md:text-sm text-red-600 hover:bg-red-50 flex items-center space-x-2"
          >
            <svg className="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
